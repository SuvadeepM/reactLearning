...
# 🧠 React 18 – Understanding `createRoot()`

In React 18+, `createRoot()` replaces the older `ReactDOM.render()` method. It enables the use of **Concurrent Mode**, which allows React to be more responsive by interrupting, pausing, and resuming rendering work.

---

## ✅ What is `createRoot()`?

`createRoot()` is part of the **React 18 concurrent rendering engine**. It initializes a **root container** that controls how React renders components into the DOM.
...
const root = createRoot(container); // Initialize root container
root.render(<App />);               // Render the App component
```

`createRoot()` → initializes container

`render(<App />)` → schedules update

React builds virtual DOM & Fiber tree

Performs diffing and reconciliation

Commits changes to real DOM


# ⚙️ Internal Working of createRoot()
## When you call `createRoot()`:

### 1. Creates a ReactRoot container

Wraps the DOM node and provides the root context for React rendering.

### 2. Initializes a Fiber Root

React's internal "fiber tree" is a data structure used for efficient diffing and scheduling.

Every React element becomes a node in this tree.

### 3. Enables Concurrent Features

React can interrupt rendering if the browser needs to do something more urgent.

Helps in implementing `startTransition`, Suspense, selective hydration, etc.

### 4. Performs Reconciliation and Commit

Virtual DOM is compared with previous state (diffing).

Minimal updates are pushed to the real DOM.

## ✅ Summary
`createRoot()`	Initializes the concurrent rendering container

`.render(<App />)`	Schedules and starts rendering

Fiber Tree	Internal data structure React uses to manage rendering

Concurrent Mode	Enables React to pause, interrupt, and resume rendering efficiently

# Shoutout to Andrew Clark - acdlite
